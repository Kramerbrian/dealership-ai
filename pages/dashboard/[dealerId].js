...
import {
  ...
  BarChart3, RefreshCw, ChevronDown, ChevronUp, ExternalLink, Lock, MessageSquare, Bot
} from "lucide-react";
...
// Removed TypeScript definitions for Next.js compatibility

const Card = ({ className = "", children }) => (
  <div className={`bg-slate-800 rounded-lg border border-slate-700 ${className}`}>{children}</div>
);

const SectionHeader = ({ title, onToggle, expanded }) => (
  <div
...
const MetricCard = ({ title, value, change, icon, footer, isLoading }) => (
...
export default function DealershipAIDashboard() {
  const [userTier, setUserTier] = useState(typeof window !== 'undefined' ? localStorage.getItem("tier") || "Level 1" : "Level 1");
  const [selectedDealership, setSelectedDealership] = useState(typeof window !== 'undefined' ? localStorage.getItem("dealer") || "Toyota of Naples" : "Toyota of Naples");
  const [selectedLocation, setSelectedLocation] = useState(typeof window !== 'undefined' ? localStorage.getItem("location") || "Naples, FL" : "Naples, FL");
  const [dealershipUrl, setDealershipUrl] = useState(typeof window !== 'undefined' ? localStorage.getItem("url") || "https://toyotaofnaples.com" : "https://toyotaofnaples.com");

  const [activeTab, setActiveTab] = useState(typeof window !== 'undefined' ? localStorage.getItem("tab") || "risk-assessment" : "risk-assessment");
  ...
  const [recommendations] = useState(defaultRecs);
  ...
  const tabs = useMemo(() => {
    if (userTier === "Level 1") {
      return [...baseTabs, { id: "upgrades", label: "Upgrades", icon: Lock }];
    }
  ...
  const jumpToRecommendations = () => {
    ...
    setTimeout(() => recsRef.current && recsRef.current.scrollIntoView({ behavior: "smooth", block: "start" }), 0);
  };

  const jumpToUpgrades = () => {
    if (userTier === "Level 1") {
      setActiveTab("upgrades");
      setTimeout(() => upgradesRef.current && upgradesRef.current.scrollIntoView({ behavior: "smooth", block: "start" }), 0);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 pb-20">{/* pb for mobile bar */}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 bg-slate-900" role="main">
        ...
        {activeTab === "risk-assessment" && (
          <div className="space-y-8" id="panel-risk-assessment" role="tabpanel" aria-labelledby="risk-assessment-tab">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <span className="px-2 py-1 bg-green-800 text-green-200 rounded text-xs">✅ ChatGPT: 92% visibility</span>
                    <span className="px-2 py-1 bg-green-800 text-green-200 rounded text-xs">✅ Perplexity: 91% visibility</span>
                    <span className="px-2 py-1 bg-green-800 text-green-200 rounded text-xs">✅ Gemini: 94% visibility</span>
                    <span className="px-2 py-1 bg-green-800 text-green-200 rounded text-xs">✅ Copilot: 88% visibility</span>
                  </div>
                </div>
              </div>
            </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <MetricCard title="Authority Score" value={`${dashboard.riskScore}/100`} change={+72} icon={<Shield className="w-6 h-6 text-green-400" />} footer={<span className="text-xs text-green-400">🎆 Maximum Score Achieved</span>} />
              <MetricCard title="Annual Revenue Gain" value={`+$63k/yr`} change={+1160} icon={<BarChart3 className="w-6 h-6 text-green-400" />} footer={<span className="text-xs text-green-400">1,160% ROI</span>} />
              <MetricCard title="AI Visibility Score" value={`${dashboard.aiVisibilityScore}%`} change={+66} icon={<Brain className="w-6 h-6 text-green-400" />} footer={<span className="text-xs text-green-400">Perfect Visibility</span>} />
              <MetricCard title="Market Position" value={`#${dashboard.marketPosition} of ${dashboard.totalCompetitors}`} change={+6} icon={<Target className="w-6 h-6 text-gold-400" />} footer={<span className="text-xs text-yellow-400">🏆 #1 Position</span>} />
            </div>
        ...
        {activeTab === "agentic-seo" && (
          <div id="panel-agentic-seo" role="tabpanel" aria-labelledby="agentic-seo-tab">
            <AgenticDashboard />
          </div>
        )}
        ...
        {userTier === "Level 1" && activeTab === "upgrades" && (
          <Card className="p-8" ref={upgradesRef} role="tabpanel" aria-labelledby="upgrades-tab">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
              {proTabs.map((p) => (
                <div key={p.id} className="border border-slate-600 rounded-lg p-4 bg-slate-800/60">
